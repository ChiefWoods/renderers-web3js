...
import { Fragment, fragment, mergeFragments, NameApi, use } from '../utils';

export function getTypeDiscriminatedUnionHelpersFragment(scope: {
    name: string;
    nameApi: NameApi;
    typeNode: TypeNode;
}): Fragment | undefined {
    ...
    const getVariantContentType = use('type GetDiscriminatedUnionVariantContent', 'codecs');
    const getVariantType = use('type GetDiscriminatedUnionVariant', 'codecs');
